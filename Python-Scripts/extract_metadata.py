...
ordering = ["filename", "au-name", "title", "au-gender", "firsted-yr", "printSource-yr", "form", "spelling", "data-capture", "token count"]
    ...
    for teiFile in glob.glob(teiFolder): 
        ...
        try: 
            if "schemas" not in filename:
                ...
                count = get_count(txt)
                #print(au_gender)
                keys.extend(["au-name", "au-gender", "token count"])
                metadata.extend([name, au_gender, count])
